...
then
    ...
    do
        ...
        # all fine, spawn background thread that waits and calls perf
        {
            ...
            export PERF_COLLAPSE_OPTS="--inline --context"
            export PERF_RECORD_FREQ=500
            export PERF_SCRIPT_OPTS="--no-demangle"
            perf-java-flames $pid --call-graph fp $opts &>> profile.log
...
java $OPTS lsr.paxos.test.GenericJniService "$@" 2>&1 | ./process.sh
